...
import { useAuth } from '../utils/AuthProvider';
    ...
    const fetchBooks = async (page = 1, limit = 9, token : string | null): Promise<{ data: BookType[], total: number, currentPage: number }> => { // Asumsi API mengembalikan total & halaman
    ...
    const response = await AxiosInstance.get<{ data: BookType[], total: number, currentPage: number }>(`/api/books?page=${page}&limit=${limit}`, {
        headers : {
    ...
    const updateBook = async (bookData: UpdateBookPayload & { id: number }): Promise<BookType> => {
